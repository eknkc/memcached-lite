...
Connection.prototype._attachEvents = function() {
  ...
  this.stream.on('error', function (msg) {
    this.emit('error', new Error("memcached-lite connection to " + this.host.string + " failed: " + msg));
    this._connectionLost('error');
  }.bind(this))

  this.stream.on('data', function (data) {
    this.reader.read(data).forEach(function (data) {
      var handler = this.handlers[data.opaque];

      if (handler) {
        delete this.handlers[data.opaque];
        handler(null, data);
      } else
        this.emit('error', 'Unexpected packet received from server.');
    }.bind(this));
  }.bind(this));
};
